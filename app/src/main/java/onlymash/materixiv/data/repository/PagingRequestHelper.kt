...
class PagingRequestHelper {
        ...
        mutex.withLock {
            ...
            when (status) {
                ...
                Status.REFRESH_TOKEN -> {
                    queue.failed = null
                    queue.status = Status.REFRESH_TOKEN
                }
                else -> {
    ...
    class RequestWrapper(
        ...
        suspend fun run() {
            withContext(Dispatchers.IO) {
                handleCallback.invoke(Callback(this@RequestWrapper, helper))
            }
        }
    ...
    class Callback (
        ...
        suspend fun recordSuccess() {
            if (called.compareAndSet(false, true)) {
                helper.recordResult(wrapper, Status.SUCCESS, null)
            } else {
                throw IllegalStateException(
                    "already called recordSuccess, recordFailure or recordRefreshToken"
                )
            }
        }

        suspend fun recordRefreshToken() {
            if (called.compareAndSet(false, true)) {
                helper.recordResult(wrapper, Status.REFRESH_TOKEN, null)
            } else {
                throw IllegalStateException(
                    "already called recordSuccess, recordFailure or recordRefreshToken"
                )
        ...
        suspend fun recordFailure(throwable: Throwable) {
            if (called.compareAndSet(false, true)) {
                helper.recordResult(wrapper,Status.FAILED, throwable)
            } else {
                throw IllegalStateException(
                    "already called recordSuccess, recordFailure or recordRefreshToken"
                )
    ...
    class StatusReport internal constructor(
        ...
        /**
         * Convenience method to check if there are any requests that token need refresh
         *
         * @return True if there are need refresh token, false otherwise.
         */
        fun needRefreshToken(): Boolean {
            return initial == Status.REFRESH_TOKEN || before == Status.REFRESH_TOKEN || after == Status.REFRESH_TOKEN
        }
